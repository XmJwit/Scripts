...
if _exists curl && [ "${ACME_USE_WGET:-0}" = "0" ]; then
  curl https://cdn.jsdelivr.net/gh/acmesh-official/acme.sh@latest/acme.sh | sh -s -- --install-online $_email "$@"
elif _exists wget ; then
  wget -O -  https://cdn.jsdelivr.net/gh/acmesh-official/acme.sh@latest/acme.sh | sh -s -- --install-online $_email "$@"
else
